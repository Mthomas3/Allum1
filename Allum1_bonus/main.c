...
** Last update Thu Feb 19 03:08:30 2015 Thomas Martins
*/
...
int		check_jcj(t_jcj *jcj, t_alum *am)
{
  ...
  my_putstr("Saisir le nombre de joueur 1 ou 2: ");
  if (!(ask = get_next_line(0)))
    return (-1);
  jcj->nb = my_getnbr(ask);
  if (jcj->nb < 1 || jcj->nb > 2)
    {
      my_puterror("Seulement un ou deux pour le nombre de joueur\n");
      exit(0);
    }
  if (jcj->nb == 1)
    return (2);
  if (jcj->nb == 2)
    if ((start_game_call(am, jcj)) < 0)
      return (-1);
  my_putstr("\033[0m");
  return (0);
}
